...
data class TetrisSpirits(
    ...
    fun adjustOffset(matrix: Pair<Int, Int>, adjustY: Boolean = true): TetrisSpirits {
        val yOffset =
            if (adjustY)
...
fun TetrisSpirits.isValidInMatrix(blocks: List<Brick>, matrix: Pair<Int, Int>): Boolean {
...
fun generateSpiritReverse(matrix: Pair<Int, Int>): List<TetrisSpirits> {
    return SpiritType.map {
        TetrisSpirits(it, Offset(Random.nextInt(matrix.first - 1), -1)).adjustOffset(matrix, false)
    }.shuffled()
}